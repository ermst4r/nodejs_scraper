...
var websiteName = "braddeals_us";
var media_ids = require('../media_ids/spain');
var websiteUrl = 'http://www.bradsdeals.com';
...
var finalActionExpireDate = ('0' + MyDate.getDate()).slice(-2) + '-'
    ...
    + MyDate.getFullYear();
var Braddeals_us = function () {
    this.fetchData = function () {
        request({
            uri: "http://www.bradsdeals.com/stores/all/"
        }, function(error, response, body) {
            var c = cheerio.load(body);
            c(".reset.listing.merchants.clearfix a").each(function() {
                var category = c(this);
                ...
                request({
                    ...
                    if(!pageError && pageResponse.statusCode==200) {
                        ...
                        d("#sales-section li.block.coupon ").each(function() {
                            var detail = d(this);
